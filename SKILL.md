...
5. **换动作时不要传空 sets**：切换到新动作时，`log` 不能只写 `{"name":"新动作","sets":[]}` —— 这会触发脚本误判「计划动作全部完成」而自动结束计时。正确做法：
   - 切换新动作前，先确认上一动作所有组都已记录
   - 切换时直接在新动作加上第一组的 reps 数据
   - 如果必须先记录动作名再记组数，用 `start-session` 重新开启一个新计时会话，再 `finish-workout` 时手动合并记录
